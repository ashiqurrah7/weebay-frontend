...
const EditProduct = () => {
  ...
  const {user, product, getProduct, editProduct} = store.commonStore;
  type RouteParams = {
    id: string;
  };
    ...
    getProduct(parseInt(id));
      if(product) {
        setValues({
          ...
          setCategories(product.categories.split(", "))
      }
  },[id, user])
  ...
  const handleChange =
    ...
    const handleSubmit=(e: FormEvent) => {
      e.preventDefault();
      if(user && product) editProduct(parseInt(id), {...values, user_id:user.id, views:product.views});
    }

  return (user && product) ? <form className="AddProductContainer" onSubmit={handleSubmit}>
